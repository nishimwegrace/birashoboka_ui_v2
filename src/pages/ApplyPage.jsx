...
export const ApplyPage = ({
  ...
  // Whether the student still needs to pick a campaign before proceeding.
  // Skipped automatically when there's only one open campaign to choose from.
  const [campaignConfirmed, setCampaignConfirmed] = useState(openCampaigns.length <= 1);
  ...
  return (
    <div className="w-full bg-slate-50 min-h-screen py-10 px-4 sm:px-6 lg:px-8">
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        {/* Section 0: Campaign Selection Step — shown first whenever there's a choice to make */}
        {openCampaigns.length > 1 && !campaignConfirmed && (
          <div className="bg-white rounded-3xl p-6 sm:p-10 border border-slate-200 shadow-xl space-y-6">
            <div className="text-center space-y-2">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-100 text-blue-700 text-xs font-black uppercase tracking-wider">
                Step 1 of 2
              </span>
              <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-900">
                Choose Your Admission Campaign
              </h2>
              <p className="text-sm sm:text-base text-slate-600 max-w-2xl mx-auto font-normal">
                There are currently {openCampaigns.length} cohorts open for registration. Select the one that matches where and when you want to train — you can change this later if needed.
              </p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              {openCampaigns.map((c) => {
                const isSelected = Number(selectedCampaignId) === c.id;
                return (
                  <button
                    key={c.id}
                    type="button"
                    onClick={() => handleCampaignChange(c.id)}
                    className={`text-left p-5 rounded-2xl border-2 transition cursor-pointer relative ${
                      isSelected
                  ...
                  >
                    {isSelected && (
                      <span className="absolute top-4 right-4 w-6 h-6 rounded-full bg-blue-600 text-white flex items-center justify-center">
                        <Check className="w-4 h-4" />
                      </span>
                    )}
                    <span className="inline-flex items-center gap-2 px-2.5 py-1 rounded-full bg-amber-100 text-amber-800 text-[11px] font-black uppercase tracking-wider mb-3">
            ...
            <div className="flex justify-center pt-2">
              <button
                type="button"
                onClick={() => setCampaignConfirmed(true)}
                className="inline-flex items-center justify-center gap-2 px-8 py-3.5 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold text-base shadow-lg shadow-blue-600/30 transition-transform active:scale-98 cursor-pointer"
              >
                <span>Continue with {activeCampaign?.edition || 'this campaign'}</span>
